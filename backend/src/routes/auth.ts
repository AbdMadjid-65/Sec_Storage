...
import { sendOtpEmail, sendPasswordResetEmail } from '../helpers/emailService';
...
router.post('/verify-reset-code', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (userResult.rows.length === 0) { res.status(400).json({ error: 'Invalid email or code' }); return; }

        const userId = userResult.rows[0].id;

        const otpResult = await pool.query(
            `SELECT id FROM otp_codes
             WHERE user_id = $1 AND code = $2 AND type = 'password_reset' AND used = FALSE AND expires_at > NOW()
             ORDER BY created_at DESC LIMIT 1`,
            [userId, code]
        );
        if (otpResult.rows.length === 0) { res.status(400).json({ error: 'Invalid or expired code' }); return; }
